...
public abstract class JdbcBaseDao <T extends Object> implements CommonDao <T> {
	...
	public int persist(T obj) {
		...
		if (values.size() != expectedCount) {
			throw new IllegalArgumentException("Inserting a row into " + TABLE_NAME + " requires exactly " + expectedCount + " field values (excludes a primary key). The call to getNVPairs(" + obj.getClass().getName() + ") implementation returned " + values.size() + " field values.");
		}
	...
	protected void initOnce(SdkDataSourceImpl dataSource, String tableName, List <String> expectedColNames) {
		...
		if (dbFieldNames.size()> 0) {
			...
			if (!pass) {
				...
				throw new IllegalStateException("In table " + tableName + ": " + causeMsg);
			}
		}
	}
